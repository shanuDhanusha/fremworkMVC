<?php require_once APPROOT.'/views/inc/inheadtecno.php';?>


    <div class="contener-tecno">
           <?php require_once APPROOT.'/views/inc/navbar.php';?>    

            <div class="contener-tecno-2">
                 <?php require_once APPROOT.'/views/inc/sidebar.php';?>
                
                 <!-- add tecno start div conterner  -->
                <div class="addtecno-div">
                    <div class="add-item-div"><a href="<?php echo 'additem';?>"><i class="fa-solid fa-layer-group"></i>Add Item</a></div>
                        <div class="pri-view-tecno">

                             <div class="tecno-title-profile-filed">
                                    <div class="profil-owner">
                                       <div class="imge-oner"><img src="../public/image/madu.jpeg" alt=""></div>
                                        <h6>13.11.2023 <br>time:5:30</h6>
                                     </div>
                                 <div class="title-tecno"><h3>Categoly:</h3>
                                       <h3>bonsi</h3>
                                   </div>
                                 
                             </div>
                                 <h5>bonsi plants</h5>
                                 <a href="">Delete...</a>
                                <div class="content">
                                    
                                    <p><?php echo 'my best palnt is bonsi we can sell more prices we can try to learn this subject sdsafghjgcvxvsgfhyghjjka afsfgygHC adsagydgadjabtgwdfgjhjkxcvjkhudfghuisdhjgdshjgh dhsfisghduahgiash ';?></p>
                                 </div>


                                  <div class="tecno-poto">

                                            <div></div>
                                            <div></div>
                                            <div></div>
                                    </div> 
                               
                               

                         </div>
                        

                 </div>


              </div>         



    </div>  




<?php require_once APPROOT.'/views/inc/infootertecno.php';?>


